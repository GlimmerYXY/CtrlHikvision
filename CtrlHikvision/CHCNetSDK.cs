...
namespace CtrlHikvision
{
    ...
    public class CHCNetSDK
    {
        ...
        **************************************************/
        ...
        public const int COMM_UPLOAD_VIDEO_INTERCOM_EVENT = 0x1132;//可视对讲事件记录信息
        public const int COMM_ALARM_VIDEO_INTERCOM = 0x1133;//可视对讲报警信息
        ...
        **************************************************/
        ...
        public struct NET_DVR_COMPRESSION_INFO_EX
        {
            ...
            // 13-384K 14-448K 15-512K 16-640K 17-768K 18-896K 19-1024K 20-1280K 21-1536K 22-1792K 23-2048K
            //最高位(31位)置成1表示是自定义码流, 0-30位表示码流值(MIN-32K MAX-8192K)。
            public uint dwVideoFrameRate;//帧率 0-全部; 1-1/16; 2-f1/8; 3-1/4; 4-1/2; 5-1; 6-2; 7-4; 8-6; 9-8; 10-10; 11-12; 12-16; 13-20, //V2.0增加14-15, 15-18, 16-22;
            public ushort wIntervalFrameI;//I帧间隔
            //2006-08-11 增加单P帧的配置接口，可以改善实时流延时问题
            public byte byIntervalBPFrame;//0-BBP帧; 1-BP帧; 2-单P帧
            public byte byRes;
        ...
        public struct NET_DVR_VIDEO_INTERCOM_EVENT
        {
            ...
            public byte byEventType; //事件信息类型，1-开锁记录，2-公告信息阅读回执，3-认证记录，4-车牌信息上传，5-非法卡刷卡事件，6-门口机发卡记录(需要启动门口机发卡功能，刷卡时才会上传该事件)，20-门磁状态事件
        ...
        {
            public byte byDoorStatus; //门状态，// 0-关门; 1-开门
        ...
        public struct NET_DVR_UNLOCK_RECORD_INFO
        {
            ...
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 202, ArraySubType = UnmanagedType.I1)]
            public byte[] byRes;//保留,置0
            public void Init()
            {
                byRes = new byte[202];
        ...
        **********************************************************/
        [DllImport(@"C:\hikvision64\HCNetSDK.dll")]
        ...
        ******************************************************************************/
        ...
        [DllImportAttribute(@"C:\hikvision64\HCNetSDK.dll")]
        ...
        public struct NET_DVR_CONTROL_GATEWAY
        {
            ...
            public byte byCommand;  //操作命令：0- 关闭，1- 打开
            public byte byLockType; //锁类型：0- 普通（不支持则默认为0），1- 智能锁
            public ushort wLockID;  //锁ID，从1开始（远程开门口机锁时，0-表示门口机本机控制器上接的锁、1-表示外接控制器上接的锁）
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = NAME_LEN)]
            public byte[] byControlSrc; //操作发起源信息
            public byte byControlType;  //开锁场景类型：1- 监视，2- 通话
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            public byte[] byRes3;   //保留，置为0
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = PASSWD_LEN)]
            public byte[] byPassword;   //锁密码，当byLockType为“1-智能锁”时有效
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 108)]
            public byte[] byRes2;   //保留，置为0
        }

        /* The remote control function
         * [in] lUserID - NET_DVR_Login_V40等登录接口的返回值
         * [in] dwCommand - 控制命令，详见列表 NET_DVR_REMOTECONTROL_GATEWAY：16009
         * [in] lpInBuffer - 输入参数，具体内容跟控制命令相关，详见列表 NET_DVR_CONTROL_GATEWAY
